...
int main(int argc, char *argv[]) {
    ...
    for (size_t i = 0; auto const & e : tests) {
        ...
        fmt::println("Test {0:2d}: simple_parse_float(\"{1:s}\"){2:<{3}s} = \tres1: {4} res_f1: {5:+010.5f} {6}",
            ++i, e.input_, " ", std::clamp((size_t)1, 20 - e.input_.length(), (size_t)20), res1, res_f1, (pass?"true":"false"));
    }
    constexpr size_t TEST_CNT = 100;
    std::mt19937_64 rnd{std::random_device()()};
    constexpr auto s = sizeof(unsigned long int);
    for(size_t i = 0; i < TEST_CNT; ++i) {
        auto r = rnd();
        float f = *reinterpret_cast<float const *>(&r);
        std::ostringstream oss;
        oss << std::fixed << f;
        ...
        if (res != 0 || !float_eq(result, f)) {
            fmt::println("Input: {:.f} Output {:.f} retval: {}", f, result, res);
        }
    }
    return 0;
}
